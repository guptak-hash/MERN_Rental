...
const Navbar = () => {
    ...
    const changeRole = async () => {
        try {
            const { data } = await axios.post('/api/change-role')
            if (data.success) {
                setIsOwner(true)
        ...
        right-0 flex flex-col sm:flex-row items-start sm-items-center gap-4 sm:gap-8 max-sm:p-4 transition-all
         duration-300 z-50 ${location.pathname === '/' ? 'bg-light' : 'bg-white'} ${open ? 'max-sm:translate-x-0' : 'max-sm:translate-x-full'}`}>
                ...
                <div className='flex max-sm:flex-col items-start sm:items-center gap-6'>
                    ...
                    <button onClick={() => { user ? logout() : setShowLogin(true) }} className='cursor-pointer px-8 py-y
                 bg-primary hover:bg-primary-dull transition-all text-white rounded-lg'>{user ? 'Logout' : "Login"}</button>
                </div>
            </div>
